...
- [x] **SYNC-18**: Offboarding support — revoke credential, remove container, delete propagated copies

### Security

- [x] **SEC-01**: Read-only mailbox.org access — never create, modify, or delete source events
- [x] **SEC-02**: Credentials encrypted at rest; never plaintext in config or version control
...
- [x] **SEC-05**: Secrets retrieval abstracted behind accessor (interim sops/age → target Vault/OpenBao)
...
- [x] **TEST-05**: CI runs tests without live provider credentials

### Operations

- [x] **OPS-01**: Single-account pilot configuration (operator calendar only)
- [x] **OPS-02**: Host migration (laptop → server) is a deployment change, not a rewrite
